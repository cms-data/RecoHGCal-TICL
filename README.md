...
- `Common input tensor`: Both models share the same initial input tensor, dimensions batch x 50 (layers) x 10 (clusters) x 7 (features).
  ...
  - `ticlv5/CNN/patternrecognition/id_v*.onnx`:
    - `v0`: Simple CNN-based approach "the same used in TICLv4", trained at the clu3d level. The neutral pion, neutral hadron, ambiguous and unknown probabilities are set to a constant value of 0.  Input and output tensors:
    - `"input"`: Input tensor with dimension `batch x 50 (layers) x 10 (clusters) x 3 (features)`.
